// Document Scanner Application - JavaScript Implementation
class DocumentScannerApp {
    ...
    handleFileSelect(event) {
        ...
        if (file.type.startsWith('image/')) {
            ...
            reader.onload = (e) => {
                this.compressAndSetImage(e.target.result);
            };
    ...
    compressAndSetImage(dataUrl) {
        ...
        img.onload = () => {
            ...
            // Redimensionar se a imagem for muito grande (máximo 1920px)
            const maxWidth = 1920;
            ...
            this.imagePreview.src = compressedDataUrl;
            this.base64Image = compressedDataUrl.split(',')[1];
    ...
    async handlePdfFile(file) {
        ...
        reader.onload = async (e) => {
            try {
                ...
                this.compressAndSetImage(imageDataUrl);
                this.imagePreviewContainer.classList.remove('hidden');
                this.extractButton.disabled = false;
            } catch (error) {
    ...
    async handleExtractData() {
        ...
        try {
            // Enviar imagem para o servidor processar (chave da API fica segura no backend)
            const response = await fetch('/api/extract', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ imageData: this.base64Image })
            });
...
if (typeof module !== 'undefined' && module.exports) {
    module.exports = DocumentScannerApp;
}